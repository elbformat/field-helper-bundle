...
class MatrixFieldHelper extends AbstractFieldHelper
{
    ...
    public function getArray(Content $content, string $fieldName): array
    {
        ...
        /** @var Value\Row $row */
        foreach ($rows as $row) {
            $result[] = array_values($row->getCells());
        }
    ...
    protected function getHeadlineIds(Content $content, string $fieldName): array
    {
        ...
        $colIds = [];
        foreach($fieldDef->fieldSettings['columns'] as $colDef) {
            $colIds[] = $colDef['identifier'];
        }
        return $colIds;
    }
    ...
    {
        return 1 > count(
                array_udiff($arr1, $arr2, function ($v1, $v2) {
                    if (is_array($v1) && is_array($v2)) {
                        return !$this->arrayContains($v1, $v2);
                    }
                    if (is_array($v1) || is_array($v2)) {
                        return 1;
                    }
